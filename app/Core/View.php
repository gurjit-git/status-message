<?php
namespace Status_Msg\Core;

use Status_Msg\Models\Config_Model;
...
class View {
    ...
    public function the_view( $file, $view_dir = null ) {
        ...
        $view_dir  = isset( $view_dir ) ? $view_dir : STATUS_MSG_PLUGIN_PATH . '/views/';
    ...
    public function get_view( $file, $view_dir = null ) {
        ...
        $view_dir  = isset( $view_dir ) ? $view_dir : STATUS_MSG_PLUGIN_PATH . '/view/';
        $view_file = $view_dir . $file . '.php';
        if ( ! file_exists( $view_file ) ) {
            return '';
        }
